...
const proxyUrl = "http://lum-customer-hl_ef8aa9eb-zone-static:7ur17zhmygqw@zproxy.lum-superproxy.io:22225";

const proxiedRequest = request.defaults({'proxy': proxyUrl});

const doReq = (restaurant) => new Promise((resolve, reject) => {

    let headers = {
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8',
        'User-agent': randomUseragent[Math.floor(Math.random() * (randomUseragent.length))],
    };

    let jar = proxiedRequest.jar();

    proxiedRequest.get({url: 'http://www.google.ro', jar, headers}, async (err, httpResponse, body) => {
        ...
        let cookie = jar.getCookieString('http://www.google.ro');
        if(cookie)
        {
            headers['Cookie'] = cookie;
            console.log("Got cookie from http://www.google.ro");
        }

        let searchTerm = "webdollar"; //what is being googled
        let target = "twitter"; //website or whatever keyword to look for in the results (can be a link)

        let maxPg = 10; //maximum number of pages to iterate through
        for(let pgIterator = 0; pgIterator < maxPg; ++pgIterator)
        {
            await wait(3); //sleeping between diff pages requests
            const res = await ((restaurant) => new Promise((resolve, reject) => {
                proxiedRequest.get({url: 'http://www.google.ro/search?q=' + searchTerm + '&start=' + 10 * pgIterator, jar, headers}, (err, httpResponse, body) => {
                    if(err)
                    {
                        reject(err);
                        return;
                    ...
                    {
                        headers['Cookie'] = cookie;
                        console.log('Got cookie from http://www.google.ro/search?q=' + searchTerm + '&start=' + 10 * pgIterator);
                    }

                    const dom = new JSDOM(body);
                    let g_list = dom.window.document.getElementsByClassName("r");
                    for(let resItr = 0; resItr < g_list.length; ++resItr)
                    {
                        try{
                            let href = g_list[resItr].querySelector("a").href;
                            console.log(resItr + ". " + href);
            ...
            {
                //found
                proxiedRequest.get({url: res, jar, headers}, (err, httpResponse, body) => {
                    if (err) {
                        reject(err);
                        return;
                    }
                    resolve("Successfully reached " + target);
                    return;
                });
                return;
            }
...
const wait = (duration) => new Promise((resolve, reject) => {
    console.log('------Sleeping for: ' + duration + ' sec------');
    setTimeout(() => {
        resolve();
    }, duration * 1000);
});
...
const run = async () => {
    await doReq('73').then((response) => {
        console.log(response)
    }).catch((err) => {
        console.log(err);
    });
    await wait(10); //in seconds
    await run();
};
